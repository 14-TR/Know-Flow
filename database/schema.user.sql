-- Know-Flow User Database Schema (SQLite)
-- This schema is for non-admin users who track projects against admin-defined process templates.
-- Process templates (processes, nodes, edges) are accessed via views attached to admin DB.

-- Projects table: instances of a process for tracking
CREATE TABLE IF NOT EXISTS projects (
    id TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(4)) || '-' || hex(randomblob(2)) || '-4' || substr(hex(randomblob(2)),2) || '-' || substr('89ab',abs(random()) % 4 + 1, 1) || substr(hex(randomblob(2)),2) || '-' || hex(randomblob(6)))),
    name TEXT NOT NULL,
    process_id TEXT NOT NULL,  -- References admin.processes (no FK since it's a view)
...
CREATE TABLE IF NOT EXISTS project_node_statuses (
    ...
    node_id TEXT NOT NULL,  -- References admin.nodes (no FK since it's a view)
...
CREATE TABLE IF NOT EXISTS project_edge_traversals (
    id TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(4)) || '-' || hex(randomblob(2)) || '-4' || substr(hex(randomblob(2)),2) || '-' || substr('89ab',abs(random()) % 4 + 1, 1) || substr(hex(randomblob(2)),2) || '-' || hex(randomblob(6)))),
    project_id TEXT NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
    edge_id TEXT NOT NULL,  -- References admin.edges (no FK since it's a view)
